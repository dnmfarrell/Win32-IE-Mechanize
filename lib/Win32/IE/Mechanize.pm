...
sub close { $_[0]->{agent}->quit; $_[0]->{agent} = undef; 1 }
...
The following tags are treated as links::
...
sub _extract_links {
    ...
    for ( my $i = 0; $i < $doc->all->length; $i++ ) {
        my $obj = $doc->all( $i );
        next unless $obj->tagName =~ /^(?:A|IFRAME|FRAME|AREA)$/i;
        print "Matched " . $obj->tagName . "\n";
